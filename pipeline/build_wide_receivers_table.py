"""Build a WR-season table joining core stats, tracking/advanced metrics,
team/coaching context, and background info from every relevant source in
data/*.parquet. One row per (player, season). Writes data/wide_receivers.parquet
...
just joins what's already cached):
    python3 pipeline/build_wide_receivers_table.py

Grain and population: a row exists only for players who (a) are tagged
position='WR' in seasonal_rosters and (b) had a target or a carry as a WR
that year (per pbp) — a WR on the roster who never touched the ball has
nothing to show here and is excluded.

Core box-score stats (receiving AND rushing — jet sweeps/end-arounds are
common enough for a WR to be worth including, same as this table's sibling
scripts) are computed directly from pbp.parquet rather than nflverse's own
seasonal.parquet, for the same reason as the quarterbacks table: that
release has stalled (no 2025 player_stats file published by nflverse as of
this writing, confirmed against the nflverse-data GitHub release directly),
while pbp/schedules/rosters are current. See build_quarterbacks_table.py's
...
re-confirmed against nflverse for running backs when that table was built):
  ...
  - fantasy_points is standard scoring; fantasy_points_ppr adds 1 point per
    ...
    a WR, same as for the running backs table.
...
why each of these has real duplicates in the raw data):
  - seasonal_rosters : WR population, name/team/bio. Trades produce one row
...
OUT_PATH = DATA_DIR / "wide_receivers.parquet"
...
LEFT JOIN pfr_rush_wr pfru ON ib.pfr_id = pfru.pfr_id AND r.season = pfru.season
LEFT JOIN pfr_rec_wr pfrc ON ib.pfr_id = pfrc.pfr_id AND r.season = pfrc.season
...
LEFT JOIN combine_wr cb ON ib.pfr_id = cb.pfr_id
LEFT JOIN contract_latest ct ON r.player_id = ct.gsis_id
ORDER BY s.season, r.team, r.player_name
"""


def build_wide_receivers_table():
    con = duckdb.connect(":memory:")
    df = con.execute(QUERY).fetchdf()
    con.close()

    df.to_parquet(OUT_PATH, index=False)
    print(f"Wrote {len(df):,} WR-season rows ({df['season'].min()}-{df['season'].max()}) to {OUT_PATH}")
    return df


if __name__ == "__main__":
    build_wide_receivers_table()
