"""Build a TE-season table joining core stats, tracking/advanced metrics,
team/coaching context, and background info from every relevant source in
data/*.parquet. One row per (player, season). Writes data/tight_ends.parquet
...
just joins what's already cached):
    python3 pipeline/build_tight_ends_table.py

Grain and population: a row exists only for players who (a) are tagged
position='TE' in seasonal_rosters and (b) had a target or a carry as a TE
that year (per pbp) — a TE on the roster who never touched the ball has
nothing to show here and is excluded.

Core box-score stats (receiving AND rushing — trick-play carries are rare
for a TE but the column is kept for consistency with this table's sibling
scripts, and costs nothing when empty) are computed directly from
pbp.parquet rather than nflverse's own seasonal.parquet, for the same
reason as the quarterbacks table: that release has stalled (no 2025
player_stats file published by nflverse as of this writing, confirmed
against the nflverse-data GitHub release directly), while
pbp/schedules/rosters are current. See build_quarterbacks_table.py's
...
then carried over here since they're the same underlying pbp quirks, and
re-confirmed against nflverse for running backs when that table was built):
  ...
  - fantasy_points is standard scoring; fantasy_points_ppr adds 1 point per
    reception; fantasy_points_half_ppr adds 0.5 — these genuinely differ for
    a TE, same as for the running backs/wide receivers tables.
  - games = count of distinct regular-season weeks with a target or a carry
...
why each of these has real duplicates in the raw data):
  - seasonal_rosters : TE population, name/team/bio. Trades produce one row
...
OUT_PATH = DATA_DIR / "tight_ends.parquet"
...
LEFT JOIN pfr_rush_te pfru ON ib.pfr_id = pfru.pfr_id AND r.season = pfru.season
LEFT JOIN pfr_rec_te pfrc ON ib.pfr_id = pfrc.pfr_id AND r.season = pfrc.season
...
LEFT JOIN combine_te cb ON ib.pfr_id = cb.pfr_id
LEFT JOIN contract_latest ct ON r.player_id = ct.gsis_id
ORDER BY s.season, r.team, r.player_name
"""


def build_tight_ends_table():
    con = duckdb.connect(":memory:")
    df = con.execute(QUERY).fetchdf()
    con.close()

    df.to_parquet(OUT_PATH, index=False)
    print(f"Wrote {len(df):,} TE-season rows ({df['season'].min()}-{df['season'].max()}) to {OUT_PATH}")
    return df


if __name__ == "__main__":
    build_tight_ends_table()
